...
def run_budget_experiment(
    ...
    """
    Evaluates memory efficiency across memory budgets and cold/hot ratios:
    - k_hot in [50, 100, 250, 500]
    - ratio in [0.5, 1.0, 2.0, 3.0]
    Total budget = k_hot + int(k_hot * ratio).
    """
    torch.manual_seed(seed)
    random.seed(seed)

    k_cold = max(10, int(k_hot * ratio))
    k_total = k_hot + k_cold
    ...
    recall = 1.0 if root_restored else 0.0
    efficiency = (recall * precision) / (k_total / 1000.0)
